...
/// <summary>
/// StableStandingAgent V2 - Improved posture training
/// KEEPS 45 OBSERVATIONS (compatible with existing trained model)
/// but adds better reward shaping for:
/// - No backward arch
/// - Shoulder symmetry
/// - No side tilt
/// </summary>
public class StableStandingAgent : Agent
{
    ...
    [Header("═══════ SHOULDERS (For symmetry penalty - NOT observed) ═══════")]
    [Tooltip("B-shoulder.L or upperArm.L transform")]
    public Transform leftShoulder;

    [Tooltip("B-shoulder.R or upperArm.R transform")]
    ...
    public override void Initialize()
    {
        ...
        // Auto-assign shoulders from upper arms if not set
        if (!leftShoulder && leftUpperArm) leftShoulder = leftUpperArm.transform;
        if (!rightShoulder && rightUpperArm) rightShoulder = rightUpperArm.transform;

        float totalMass = 0f;
        foreach (var rb in allBodies)
            totalMass += rb.mass;

        Debug.Log($"[StableStandingAgent] Initialized: {allBodies.Count} rigidbodies, {totalMass:F1} kg total mass");
    ...
    /// <summary>
    /// 45 Observations total - IDENTICAL to original BalanceAgent
    /// This allows initialization from pretrained model
    /// </summary>
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // TOTAL: 10 + 7 + 4 + 6 + 6 + 2 + 6 + 4 = 45 (UNCHANGED)
    }
    ...
    private void ObserveLimb(VectorSensor sensor, Rigidbody limb)
    {
        if (limb && hips)
        {
            ...
            sensor.AddObservation(limb.angularVelocity.magnitude / 10f);
        }
        else
        {
            sensor.AddObservation(0f);
    ...
    private void CalculateRewards()
    {
        ...
        // ═══════════════════════════════════════════════════════════════════
        // POSTURE METRICS (calculated but NOT observed - rewards only)
        // ═══════════════════════════════════════════════════════════════════

        // Spine orientation
        float spineUp = spine ? Vector3.Dot(spine.transform.up, Vector3.up) : 0f;
        float spineForwardY = spine ? spine.transform.forward.y : 0f;  // Detects backward arch
        float spineRoll = spine ? Vector3.Dot(spine.transform.right, Vector3.up) : 0f; // Detects side tilt

        // Hips orientation
        float hipsUp = Vector3.Dot(hips.transform.up, Vector3.up);
        float hipsForwardY = hips.transform.forward.y;
        float hipsRoll = Vector3.Dot(hips.transform.right, Vector3.up);

        // Shoulder symmetry (calculated but not observed)
        float shoulderHeightDiff = 0f;
        if (leftShoulder && rightShoulder)
        {
            shoulderHeightDiff = Mathf.Abs(rightShoulder.position.y - leftShoulder.position.y);
        }

        // ═══════════════════════════════════════════════════════════════════
        // PRIMARY: HEIGHT REWARD
        // ═══════════════════════════════════════════════════════════════════
        float heightRatio = Mathf.Clamp01(hipHeight / targetHipHeight);
        reward += heightRatio * 0.3f;

        // ═══════════════════════════════════════════════════════════════════
        // CRITICAL: STRAIGHT POSTURE REWARD
        // ═══════════════════════════════════════════════════════════════════

        // Reward for spine pointing straight up (not arched back or forward)
        // spineForwardY should be close to 0 for straight posture
        // Range: -1 (looking down) to +1 (looking up/arched back)
        float straightSpine = 1f - Mathf.Abs(spineForwardY);  // 1.0 = perfect, 0 = fully arched
        reward += straightSpine * 0.25f;

        // Reward for no side tilt (roll should be ~0)
        float noSpineRoll = 1f - Mathf.Abs(spineRoll);
        reward += noSpineRoll * 0.15f;

        // Reward for hips level (no roll)
        float noHipsRoll = 1f - Mathf.Abs(hipsRoll);
        reward += noHipsRoll * 0.1f;

        // ═══════════════════════════════════════════════════════════════════
        // SHOULDER SYMMETRY REWARD
        // ═══════════════════════════════════════════════════════════════════
        if (leftShoulder && rightShoulder)
        {
            float symmetryReward = Mathf.Clamp01(1f - shoulderHeightDiff / 0.15f);
            reward += symmetryReward * 0.2f;
        }
        ...
        // ═══════════════════════════════════════════════════════════════════
        // BIG BONUS FOR PERFECT POSTURE
        // ═══════════════════════════════════════════════════════════════════
        bool goodHeight = heightRatio > 0.85f;
        bool straightBack = Mathf.Abs(spineForwardY) < 0.1f;
        bool noTilt = Mathf.Abs(spineRoll) < 0.1f;
        bool symmetricShoulders = shoulderHeightDiff < 0.05f || (!leftShoulder || !rightShoulder);

        if (goodHeight && straightBack && noTilt && symmetricShoulders)
        {
            reward += 0.6f;  // MAJOR bonus for perfect posture
        }
        else if (goodHeight && straightBack)
        {
            reward += 0.3f;  // Good bonus for tall + straight
        }
        else if (heightRatio > 0.75f && Mathf.Abs(spineForwardY) < 0.2f)
        {
            reward += 0.15f;
        }
        ...
        foreach (var rb in allBodies)
        {
            if (rb) angVelSum += rb.angularVelocity.magnitude;
        }
        float stability = Mathf.Clamp01(1f - (angVelSum / (allBodies.Count * 5f)));
        ...
        // === CRITICAL: BACKWARD ARCH PENALTY ===
        // spineForwardY > 0 means spine is tilted back (looking up = arched back)
        if (spineForwardY > 0.3f)
            reward -= 0.5f;   // SEVERE penalty for major backward arch
        else if (spineForwardY > 0.15f)
            reward -= 0.3f;   // Moderate penalty
        else if (spineForwardY > 0.08f)
            reward -= 0.15f;  // Small penalty

        // Forward lean penalty (less severe since it's more natural)
        if (spineForwardY < -0.4f)
            reward -= 0.3f;
        else if (spineForwardY < -0.25f)
            reward -= 0.15f;

        // === SIDE TILT PENALTY ===
        if (Mathf.Abs(spineRoll) > 0.25f)
            reward -= 0.4f;   // SEVERE penalty for major tilt
        else if (Mathf.Abs(spineRoll) > 0.12f)
            reward -= 0.25f;
        else if (Mathf.Abs(spineRoll) > 0.06f)
            reward -= 0.1f;

        // === SHOULDER ASYMMETRY PENALTY ===
        if (leftShoulder && rightShoulder)
        {
            if (shoulderHeightDiff > 0.12f)
                reward -= 0.4f;   // SEVERE: one shoulder way higher
            else if (shoulderHeightDiff > 0.07f)
                reward -= 0.25f;
            else if (shoulderHeightDiff > 0.04f)
                reward -= 0.1f;
        }

        // === HIPS TILT PENALTY ===
        if (Mathf.Abs(hipsRoll) > 0.2f)
            reward -= 0.2f;
        else if (Mathf.Abs(hipsRoll) > 0.1f)
            reward -= 0.1f;

        // Penalty for spine not upright
        if (spineUp < 0.5f)
            reward -= 0.3f;
        else if (spineUp < 0.7f)
            reward -= 0.15f;
        ...
        // Penalty for CoM far from support
        Vector3 support = (leftFoot && rightFoot) ? (leftFoot.position + rightFoot.position) * 0.5f : hips.position;
        Vector3 comGround = new Vector3(centerOfMass.x, 0, centerOfMass.z);
        Vector3 supGround = new Vector3(support.x, 0, support.z);
        float dist = Vector3.Distance(comGround, supGround);
        if (dist > 0.15f)
    ...
    public override void Heuristic(in ActionBuffers actionsOut)
    {
        ...
        if (Input.GetKey(KeyCode.UpArrow)) act[0] = -0.5f;
        if (Input.GetKey(KeyCode.DownArrow)) act[0] = 0.5f;
        if (Input.GetKey(KeyCode.LeftArrow)) act[1] = -0.5f;
        if (Input.GetKey(KeyCode.RightArrow)) act[1] = 0.5f;
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.color = ratio > 0.75f ? Color.green : Color.red;
        Gizmos.DrawLine(hips.position, hips.position + Vector3.up * 0.2f);

        // Spine forward direction (shows arch - should point forward, not up)
        if (spine)
        {
            float forwardY = spine.transform.forward.y;
            Gizmos.color = Mathf.Abs(forwardY) < 0.1f ? Color.green : Color.red;
            Gizmos.DrawRay(spine.position, spine.transform.forward * 0.4f);
            
            // Spine up (should point up)
            Gizmos.color = Color.cyan;
            Gizmos.DrawRay(spine.position, spine.transform.up * 0.3f);
        }

        // Shoulder height visualization
        if (leftShoulder && rightShoulder)
        {
            float diff = Mathf.Abs(rightShoulder.position.y - leftShoulder.position.y);
            Gizmos.color = diff < 0.05f ? Color.green : Color.red;
            Gizmos.DrawLine(leftShoulder.position, rightShoulder.position);
            Gizmos.DrawWireSphere(leftShoulder.position, 0.03f);
            Gizmos.DrawWireSphere(rightShoulder.position, 0.03f);
        }
